package org.phypo.Game.PPgGame3d;
...
public class PPgGame3d  extends World3d {
		...
		static public PPgGame3d sTheGame;
	...
	static FloatBuffer sFBuf = ByteBuffer.allocateDirect(256).asFloatBuffer();
		...
		//------------------------------------------------
		public  PPgGame3d(  DimFloat3 pSz, Engine pEngine ){
				super( pSz, pEngine );
    ...
    public static void main( String [] args ) {
				...
				Engine    lEngine = new Engine( "Test JOGL",  lW, lH,  lStrFullScreen != null, lSmooth, true  );
 				sTheGame = new PPgGame3d( new DimFloat3( 100, 100, 100 ), lEngine );

				sTheGame.cFactory = new FactoryLevel( 0 );
				sTheGame.cFactory.setBeginLevel( lBeginLevel );

				World3d.Get().addActor( new SkyDomeStar( 1000 ));

				sTheGame.addVirtualActor( FactoryLevel.sTheFactoryLevel.getCurrentLevel() );				

				HumanControl lControl =  new HumanControl( sTheGame ); 
				sTheGame.setUserControl( lControl );
